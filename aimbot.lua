...
local freeze_accuracy = true							-- When aimbot is active, freezes the accuracy until, aimbot is off.
...
local fire_delay = 0.05									-- Adds fire delay on top of weapons fire delay
local custom_damage_by_unit = false						-- Set to true to use defined damage by unit in table bellow or false.
...
local c = not loaded and rawset(_G, mod_name, {currect_fire_delay = 0, unit_base = {}}) and _G[mod_name] or loaded
...
if not loaded then
	...
	local function get_fire_delay(wep_base)
		local fire_rate_data = wep_base:recoil_wait() or 0.8
		local unit_fire_rate = fire_delay_by_unit[c.unit_base._tweak_table]
		local min_rate = (unit_fire_rate and 0.04 or fire_rate_data) and fire_delay
		local max_rate = (unit_fire_rate and 0.07 or fire_rate_data / 2 + fire_rate_data) and fire_delay
	...
	local function is_hostage(unit)
		...
		if player_sentries[unit:name():t()] or char_dmg and (char_dmg._dead or char_dmg._invulnerable or char_dmg._immortal or char_dmg._god_mode) or brain.is_hostage and brain:is_hostage() or not brain:is_hostile() then
			return true
		end
	end
	...
	local function get_target(player_unit, wep_base, tweak, special_wep)
		...
		for _, unit in pairs(World:find_units("camera_cone", camera:camera_object(), Vector3(0, 0), (fov / 180 * 2), (max_distance > 0 and max_distance or special_wep and tweak.flame_max_range or tweak.damage_near or 7000), get_mask())) do
			...
			count[#count + 1] = {position = head_pos, dir = direction}
	...
	function EnemyManager:update(t, dt)
		...
		c.unit_base = c.unit.position and c.unit.position.base and c.unit.position:base() or {}
		c.currect_fire_delay = c.currect_fire_delay + dt

		if c.unit.dir and c.unit_base and not blocked_units[c.unit_base._tweak_table] and c.currect_fire_delay >= get_fire_delay(wep_base) then
			c.currect_fire_delay = 0
	...
	function NewRaycastWeaponBase:fire(from_pos, direction, dmg_mul, shoot_player, spread_mul, autohit_mul, suppr_mul, target_unit)
		if c.active and not silent_shooting and c.unit.dir and not key_pressed() and self._setup.user_unit and (self._setup.user_unit == c.player_unit) then
			return old_fire(self, from_pos, c.unit.dir, hit_damage(c.unit_base.position, dmg_mul), shoot_player, 0, autohit_mul, suppr_mul, c.unit_base)
		end
	...
	function StatisticsManager:hit_accuracy(...)
		...
		c.accuracy = c.accuracy or acc or 100
	...
	function StatisticsManager:session_hit_accuracy(...)
		...
		c.session_accuracy = c.session_accuracy or acc or 100
